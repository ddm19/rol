...
const NavButton: React.FC<NavButtonProps> = (props: NavButtonProps) =>
{
    ...
    return (
        <li className={`navButton ${isActive(link.url) ? 'activeLink' : ''}`}>
            <Link className='navLink link' to={(isMobileDevice() && link.sublinks) ? '#' : link.url} onClick={isMobileDevice() ? (e) => handleMobileLinkClick(e, link) : () => { }}>
                {link.name}
                    ...
                    {link.sublinks.map((sublink: NavigationLink, subindex) =>
                    {
                        return (
                            <li key={subindex} className='dropdown-content'>
                                <Link className='navLink link' to={link.url + sublink.url} onClick={toggleMenu}>{sublink.name}</Link>
                            </li>
                        );
                    })}
